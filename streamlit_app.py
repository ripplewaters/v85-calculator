...
➡️ Fyll i hur många hästar du spelat i varje lopp  
➡️ Markera vilka lopp som var **rätt**  
➡️ Klicka **Beräkna** för att se resultatet
""")

st.divider()

# --- Input: antal hästar per lopp ---
antal = [st.number_input(f"**Lopp {i+1}**", min_value=1, max_value=15, value=1, step=1) for i in range(8)]

# --- Välj vilka lopp som var rätt ---
ratt_lopp = st.multiselect("Vilka lopp var rätt?", [f"L{i+1}" for i in range(8)], default=[f"L{i+1}" for i in range(8)])
...
def rakna_vinster(antal, ratt_index):
    ...
    fel_index = [i for i in range(n) if f"L{i+1}" not in ratt_index]
